...
#### **Q2:** The following code[^tib] chooses 1,000 samples from the marble jar, each of size 1000. What might you imagine this histogram of proportions of blue marbles in samples will look like? What will its center be? What shape will it be? Try running this code multiple times, with different sample sizes. What do you observe? *Answer here: [Ans: Q2]*
...
Note: just like the sampling distribution for proportions, note that the standard deviation is inversely proportion to the square root of sample size. This is why the means of samples of larger sizes vary less. (And the associated sampling distribution is narrower.)
...
[^doc]: `for` loop tutorials from @r_for and @r_for_2; `if`-`else` statement documentation/tutorial from @r_if_else. `sample` documentation/tutorials and intuition about binomial distributions from @r_sample and @r_SO1, @r_sample_3, @r_sample_4, @r_sample_5, @r_sample_2, @r_SO3, and @r_SO4; `colon` operator documentation from @r_colon; `append` documentation/tutorial from @r_append; `vector` documentation/tutorial from @r_vector; for general graph-making on all graphs, the GGplot2 documentation (@gg) and @r_hist_ref were used; for `tibble` manipulation @tibble_manip was used; for coloring @r_SO2 was used.
[^refref]: Bibliography-making documentation and other Bookdown reference from @r_bib. Most bibtex snippets generated by [www.bibme.org]. Disclaimer: I tried to be *as scrupulous as reasonably possible* in citing my sources, including looking through my internet history and sometimes including sources that didn't directly affect this article (that's why there are so many.) However, it is possible that I may have *unintentionally* left some out. In general, though, I **did not** copy sentences or code directly from websites (unless there was a specific code syntax.) Rather, I used the websites for background information on sampling distributions and R code, and reworked it for my purposes.
