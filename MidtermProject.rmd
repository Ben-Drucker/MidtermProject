...
Often, we desire statistical information about a population. These sorts of studies tend to focus on proportions or means of some aspect of the population. For example, in politics, we might wonder if the proportion of female gun-owners is 50% (i.e., is there an even gender split in American gun ownership[^gender-simplicity])? Or, we might ask if there is a difference in the mean household income between men in opposite-sex marriages vs. in same-sex marriages.
...
Let's begin a simple example: marbles in a jar. Suppose I filled a jar filled with 10,000 blue marbles and 10,000 red marbles. Thus, there are twenty thousand marbles in total. However, suppose I don't tell you this, and I task you with estimating the proportion of red marbles in the jar. It would be impractical to count all the marbles, so you decide to count a sample of them and use that as the estimate. (This is analogous to the example about the proportion of gun owners who are female!)

#### **Q1: The code below[^doc] simulates selections from the jar with samples of size 3, 10, 100, and 1,000, and displays the results in histograms. Run this code multiple times. Before running this code, what do you predict will happen when sample size increases? After running the code, did you find your hypothesis was true? *Answer here: [Ans: Q1]***
...
#### **Q2:** The following code chooses 1,000 samples from the marble jar, each of size 1000. What might you imagine this histogram of proportions of blue marbles sampled will look like? What will its center be? What shape will it be? Try running this code multiple times, with different sample sizes. What do you observe? *Answer here: [Ans: Q2]*
...
This type of histogram is called a **sampling distribution of a proportion**. This is because it is the *distribution* of *samples* from which we are trying to gain information about a population *proportion*.
...
**Q6: Read the introduction to the following article on [https://news.gallup.com/poll/264932/percentage-americans-own-guns.aspx](the study @pew about gun ownership mentioned earlier.) We want to examine the sampling distribution for this study. (Let your null hypothesis be "the proportion of gun owners who are female is 50%") First, draw out the sampling distribution indicating the center and standard deviation. Draw two vertical lines on the histogram corresponding to the sample proportion, $\hat{p}$. Qualitatively, what do you conclude?[^gun_ref]*Answer here: [Ans: Q6]***
...
#### **Q7: The following code shows histograms of marble diameters found in random samples of differing sizes. It also shows the mean of the sample. Try running the code multiple times. Verify that the mean is less variable for samples of larger sizes. *Answer here: [Ans: Q7]***
...
#### Ans: Q1
You should have found that as the sample size increases, the bar graphs are generally less variable (across different runs of the code) and closer to the true proportion of blue/red marbles in the jar. Here is an example of one such run:
...
As you can probably imagine, sampling distributions for proportions are centered at $p$, the true population proportion. This should intuitively make sense; with many random samples, we would expect the samples to average to the underlying population proportion.

#### Ans: Q4
The answer is as follows:

$$\text{mean} = 0.70$$
$$\text{SD} = \sqrt{\frac{0.7\cdot0.3}{421}} \approx 0.223$$
...
the sampling distribution is $\sqrt{\frac{0.5*0.5}{1000}} \approx 0.0158$.
...
deviation of $\sqrt{\frac{0.5*0.5}{1269}} \approx 0.014$ since the sample size was 1296.
Graph below:
```{r}
